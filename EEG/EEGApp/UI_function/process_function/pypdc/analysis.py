...
from .ar_data import ar_data
from .ar_fit import mvar
from . import asymp as as_
from .plotting import *
from . import plotting as pl_
from . import bootstrap as bt_
...
def pc_alg(A, e_cov, nf = 64):
    '''Calculates the Partial Coherence
        ...
        '''
    n, n, r = A.shape
    e_cov = np.asmatrix(e_cov)
    AL = A_to_f(A, nf)
    pc = empty(AL.shape, dtype = 'complex')
    for i in range(nf):
        ALi = np.asmatrix(AL[i])
...
def pdc(data, maxp = 30, nf = 64, detrend = True, normalize = False,
    ...
    _,er,A,*_= mvar(data, maxp, criterion=crit)
...
def ss(data, maxp = 30, nf = 64, detrend = True, normalize = False, fixp = False, ss = True):
    ...
    A, er = mvar(data, maxp, criterion=crit)[2,1]
...
def gci(data, maxp = 30, detrend = True):
    ...
    _,er0,A0,*_= mvar(data, maxp)
    va0 = diag(er0)

    gci = zeros([n,n])
    for i in arange(n):
        aux_data = delete(data, i, 0)
        _,er1,A1,*_ = mvar(aux_data, maxp)
        va1 = diag(er1)
        va1 = insert(va1, i, 0)
...
def measure_full(data, measure, maxp = 5, nf = 64, sample_f = 1,
    ...
    algo = 0
    #Estimate AR parameters with Nuttall-Strand
    _,erest,Aest,*_ = mvar(data, maxp, alg=algo,criterion=crit)
