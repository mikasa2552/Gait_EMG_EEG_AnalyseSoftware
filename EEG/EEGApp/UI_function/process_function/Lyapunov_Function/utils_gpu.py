...
def neighbors_gpu(y, metric='euclidean', window=0, maxnum=None):
    if metric == 'cityblock':
        p = 1
    elif metric == 'euclidean':
        p = 2
    ...
    n = len(y)
    dist_matrix = torch.cdist(y, y, p=p)
    indices = torch.arange(n, device=y.device)
    valid = (torch.abs(indices[:, None] - indices) > window) & (dist_matrix > 0)
    valid_dist = torch.where(valid, dist_matrix, float('inf'))
    min_dist, min_indices = torch.min(valid_dist, dim=1)
    return min_indices, min_dist  # 返回 PyTorch 张量
