...
def nstrand(u, maxp=30, simplep=True):
    ...
    Nuttall-Strand algorithm (a generalization of single channel harmonic method).

    Input parameters:
        u      - Complex matrix with NUMCHS channels of sample data
        maxp   - Order of autoregressive model (integer)
        simplep - If True, return only A and pf; otherwise, return all intermediate results.

    Output parameters:
        A      - Complex array of forward linear prediction matrix coefficients
        pf     - Covariance matrix of NUMCHS x NUMCHS of linear forward prediction error
    """
    lx, cx = u.shape
    if lx > cx:
    ...
    ef = np.array(u)  # Eq. (15.91)
    eb = np.array(u)  # Eq. (15.91)
    pf = np.dot(u, u.T)  # Eq. (15.90)
    pb = np.array(pf)  # Eq. (15.90)
    M = 0

    # Main Loop
    while True:
        # Update estimated covariance errors  Eq. (15.89)
        pfhat = np.dot(ef[:, M+1:N], ef[:, M+1:N].T)
        pbhat = np.dot(eb[:, M:N-1], eb[:, M:N-1].T)
        pfbhat = np.dot(ef[:, M+1:N], eb[:, M:N-1].T)
        M += 1

        # Calculate estimated partial correlation matrix - Eq. (15.98)
        # (Nuttall-Strand algorithm only)
        # Replace inv with pinv or regularized inv
        inv_pf = pinv(pf + 1e-10 * np.eye(pf.shape[0]))  # Regularized pseudo-inverse
        inv_pb = pinv(pb + 1e-10 * np.eye(pb.shape[0]))  # Regularized pseudo-inverse
        RHO = solve_sylvester(np.dot(pfhat, inv_pf), np.dot(inv_pb, pbhat), 2 * pfbhat)

        # Update forward and backward reflection coefficients
        # Eqs. (15.73), (15.74), (15.78) (Nuttall-Strand algorithm)
        AM = np.dot(-RHO, inv_pb)
        BM = np.dot(-RHO.T, inv_pf)

        # Initialize or resize A and B arrays
        if M == 1:
            A = np.zeros((1, AM.shape[0], AM.shape[1]), dtype=float)
            B = np.zeros((1, BM.shape[0], BM.shape[1]), dtype=float)
        else:
            A = np.resize(A, (M, AM.shape[0], AM.shape[1]))
            B = np.resize(B, (M, BM.shape[0], BM.shape[1]))

        A[M-1, :, :] = AM
        B[M-1, :, :] = BM

        # Update forward and backward covariance error - Eqs. (15.75), (15.76)
        pf = pf - np.dot(np.dot(AM, BM), pf)
        pb = pb - np.dot(np.dot(BM, AM), pb)

        # Update forward and backward predictor coefficients - Eqs. (15.84), (15.85)
        if M > 1:
            for K in range(1, M):
                temp1 = A[K-1, :, :].copy()
                A[K-1, :, :] = A[K-1, :, :] + np.dot(AM, B[M-K-1, :, :])
                B[M-K-1, :, :] = B[M-K-1, :, :] + np.dot(BM, temp1)

        # Update forward and backward prediction errors
        Tef = np.array(ef)
        ef[:, np.arange(N-1, M-1, -1)] = ef[:, np.arange(N-1, M-1, -1)] + np.dot(AM, eb[:, np.arange(N-2, M-2, -1)])
        eb[:, np.arange(N-1, M-1, -1)] = eb[:, np.arange(N-2, M-2, -1)] + np.dot(BM, Tef[:, np.arange(N-1, M-1, -1)])
    ...
    if simplep:
        return A.transpose(1, 2, 0), np.abs(pf) / N
    else:
...
def mvar(u, MaxIP = 30, alg=1, criterion=1, return_ef = False):
    ...
    if criterion == 5:    # Fixed order at IP
        ...
        vaic = max(u.shape)*log(det(pf+ 1e-10)) + 2*nChannels*nChannels*IP # Akaike's 
        Vaicv =  vaic
        # if (not return_ef):
    ...
    while IP <= UpperboundOrder:
        ...
        if criterion == 1:   # Akaike's Information Criterion (AIC)  # (4.3.2)(Lutkepöhl '85)
            ...
            vaic = max(u.shape) * log(det(npf)) + 2*log(log(len(max(u.shape))))*nChannels*nChannels*IP
        ...
        if (vaic > vaicv+1e-2) and not (IP == 1): # Akaike condition
            vaic = vaicv
